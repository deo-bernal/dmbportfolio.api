...
await using (var scope = app.Services.CreateAsyncScope())
{
    ...
    try
    {
        ...
        await dbContext.Database.ExecuteSqlRawAsync("""
            ALTER TABLE "User"
                ADD COLUMN IF NOT EXISTS "AppPinHash" VARCHAR(255);
            """);

        await dbContext.Database.ExecuteSqlRawAsync("""
            ALTER TABLE "User"
                ADD COLUMN IF NOT EXISTS "AppPinSalt" VARCHAR(255);
            """);
        ...
        await dbContext.Database.ExecuteSqlRawAsync("""
            ...
            (
                "AppRefreshTokenId" INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                "UserId" INTEGER NOT NULL,
                "RefreshTokenHash" VARCHAR(128) NOT NULL UNIQUE,
                "ExpiresAt" TIMESTAMPTZ NOT NULL,
                "RevokedAt" TIMESTAMPTZ NULL,
                "CreatedAt" TIMESTAMPTZ NOT NULL DEFAULT NOW(),
                CONSTRAINT "FK_AppRefreshToken_User"
                    FOREIGN KEY ("UserId") REFERENCES "User" ("UserId")
                    ON DELETE CASCADE
            );
            """);
        ...
        await dbContext.Database.ExecuteSqlRawAsync("""
            ...
            (
                "ExternalLoginId" INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                "UserId" INTEGER NOT NULL,
                "Provider" VARCHAR(32) NOT NULL,
                "ProviderUserId" VARCHAR(128) NOT NULL,
                "CreatedAt" TIMESTAMPTZ NOT NULL DEFAULT NOW(),
                CONSTRAINT "FK_ExternalLogin_User"
                    FOREIGN KEY ("UserId") REFERENCES "User" ("UserId")
                    ON DELETE CASCADE
            );
            """);
        ...
        await dbContext.Database.ExecuteSqlRawAsync("""
            ...
            (
                "PendingExternalLoginId" INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        ...
        await dbContext.Database.ExecuteSqlRawAsync("""
            ...
            BEGIN
                ...
                DELETE FROM "ExternalLogin" WHERE "UserId" = p_user_id;
                DELETE FROM "PendingExternalLogin" WHERE "Email" IN (
                    SELECT lower("Email") FROM "User" WHERE "UserId" = p_user_id
                );
